...
#End Region
    ...
    Private Sub frmCargos_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        deshabilitar()
        'MessageBox.Show("Proc deshabilitar")
        mostrar()
        ' MessageBox.Show("Mostrar")
        formatear_grid()
        'MessageBox.Show("Formateod egrid")
        dataListado.Focus()
    ...
    Private Sub buscar()
        Try
            ...
            If dt.Count <> 0 Then
                lblinexistente.Visible = False
                dataListado.DataSource = db.procedencia.Where(Function(p) p.Pro_Nombre.Contains(txtBuscar.Text)).ToList()
        ...
        If sw_guardar = True Then 'Guardamos si los controles no estan vacios
            ...
            If result = DialogResult.OK Then
                Try
                    ...
                    If nuevo = True Then
                        Try
                            Dim pr As New procedencia
                            pr.Pro_Nombre = txtDescripcion.Text
                            db.procedencia.AddObject(pr)
                            db.SaveChanges()
                            fauditoria.insertar("procedencia", "Insertar", pr.Pro_id, pr.Pro_Nombre)
                    ...
                    If modificar = True Then
                        Try
                            Dim m As procedencia = db.procedencia.FirstOrDefault(Function(p) p.Pro_id = txtId.Text)
                            m.Pro_Nombre = txtDescripcion.Text
                            db.SaveChanges()
                            fauditoria.insertar("procedencia", "Editar", m.Pro_id, m.Pro_Nombre)
            ...
            Try
                For Each row As DataGridViewRow In dataListado.Rows
                    ...
                    If marcado Then
                        ...
                        Try
                            Dim db As New samadEntities
                            Dim d = db.procedencia.FirstOrDefault(Function(p) p.Pro_id = onekey)
                            db.procedencia.DeleteObject(d)
                            db.SaveChanges()
                            fauditoria.insertar("procedencia", "Insertar", onekey, "")
                        Catch ex As Exception

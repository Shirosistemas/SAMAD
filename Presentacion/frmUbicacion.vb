Public Class frmUbicacion
    ...
    Private Sub mostrar()
        Try
            ...
            Dim db As New samadEntities
            dt = db.ubicacion.Where(Function(p) p.Ubi_ProId = Pro_id).ToList()

            dataListado.Columns.Item("Eliminar").Visible = False 'Ocultamos la Columna Eliminar
            ' dataListado.Columns.Item("Ubi_ProId").Visible = False
    ...
    Private Sub formatear_grid()
        ...
        dataListado.Columns(3).HeaderText = "Propietario"
    ...
    Private Sub buscar()
        Try
            ...
            If dt.Count <> 0 Then
                lblinexistente.Visible = False
                dataListado.DataSource = dt.Where(Function(p) p.Ubi_Nombre.Contains(txtBuscar.Text)).ToList()
        ...
        If sw_guardar = True Then 'Guardamos si los controles no estan vacios
            ...
            If result = DialogResult.OK Then
                Try
                    ...
                    If nuevo = True Then
                        Try
                            Dim pr As New ubicacion
                            pr.Ubi_Nombre = txtDescripcion.Text
                            pr.Ubi_ProId = Pro_id
                            db.ubicacion.AddObject(pr)
                            db.SaveChanges()
                            fauditoria.insertar("ubicacion", "Insertar", pr.Ubi_id, "")
                    ...
                    If modificar = True Then
                        Try
                            Dim m As ubicacion = db.ubicacion.FirstOrDefault(Function(p) p.Ubi_id = txtId.Text)
                            m.Ubi_Nombre = txtDescripcion.Text
                            m.Ubi_ProId = Pro_id
                            db.SaveChanges()
                            fauditoria.insertar("ubicacion", "Insertar", m.Ubi_id, "")
            ...
            Try
                For Each row As DataGridViewRow In dataListado.Rows
                    ...
                    If marcado Then
                        ...
                        Try
                            Dim db As New samadEntities
                            Dim d = db.ubicacion.FirstOrDefault(Function(p) p.Ubi_id = onekey)
                            db.ubicacion.DeleteObject(d)
                            db.SaveChanges()
                            fauditoria.insertar("ubicacion", "Eliminar", d.Ubi_id, "")
                        Catch ex As Exception

Public Class frmUnidadMedida
    ...
    Dim dt As List(Of unidad_medida_ab)
    ...
    Private Sub buscar()
        Try
            ...
            If dt.Count <> 0 Then
                lblinexistente.Visible = False
                dataListado.DataSource = db.unidad_medida_ab.Where(Function(p) p.Uni_Descripcion.Contains(txtBuscar.Text)).ToList()
        ...
        If sw_guardar = True Then 'Guardamos si los controles no estan vacios
            ...
            If result = DialogResult.OK Then
                Try
                    ...
                    If nuevo = True Then
                        Try
                            Dim pr As New unidad_medida_ab
                            pr.Uni_Abreviatura = txtAbreviatura.Text
                            pr.Uni_Descripcion = txtDescripcion.Text
                            db.unidad_medida_ab.AddObject(pr)
                            db.SaveChanges()
                            fauditoria.insertar("linea", "Insertar", pr.Uni_id, "")
                    ...
                    If modificar = True Then
                        Try
                            Dim m As unidad_medida_ab = db.unidad_medida_ab.FirstOrDefault(Function(p) p.Uni_id = txtId.Text)
                            m.Uni_Abreviatura = txtAbreviatura.Text
                            m.Uni_Descripcion = txtDescripcion.Text
                            db.SaveChanges()
                            fauditoria.insertar("linea", "Insertar", m.Uni_id, "")
            ...
            Try
                For Each row As DataGridViewRow In dataListado.Rows
                    ...
                    If marcado Then
                        ...
                        Try
                            Dim db As New samadEntities
                            Dim d = db.unidad_medida_ab.FirstOrDefault(Function(p) p.Uni_id = onekey)
                            db.unidad_medida_ab.DeleteObject(d)
                            db.SaveChanges()
                            fauditoria.insertar("linea", "Eliminar", d.Uni_id, "")
                        Catch ex As Exception
